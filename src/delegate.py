...
import compiling
from compiling import ezcompiler
...
def gen_static_lib():
	...
	ezcompiler.add_c_code(ezdata.EZC_C)
	ezcompiler.add_code(ezdata.EZC_DOGFOOD)
	...
	outf = open(fl, "w+")
	outf.write(ezcompiler.get_c_file())
	outf.close()
...
def get_lib_args():
	"""
	Returns the c ezcompiler's linking options for gmp and mpfr
	"""
...
def compile_files(sources):
	...
	for src in sources:
		if src[-4:] == ".ezc":
			addcode(open(src, "r").read(), ezcompiler)
